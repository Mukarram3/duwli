...
export default function View() {
    ...
    return (
        <AuthenticatedLayout
            ...
            pageDescription={t('View details, items, payment status, and actions for this purchase invoice.')}
                    ...
                    <Card className="border border-border shadow-md rounded-xl overflow-hidden bg-card">
                        ...
                        <CardContent className="p-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {/* Left Side: Vendor Address (Billed From) */}
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        <h3 className="text-xs font-bold tracking-wider text-muted-foreground">{t('Billed From')}</h3>
                                        <div className="space-y-1 text-sm">
                                            <div className="font-semibold text-foreground text-base">{invoice.vendor?.name}</div>
                                ...
                                {/* Right Side: Company Details (Billed To) */}
                    ...
                    <Card className="border border-border shadow-md rounded-xl overflow-hidden bg-card">
                        ...
                        <CardContent className="p-6 space-y-6">
                            <div className="space-y-1.5 text-center sm:text-start">
                                ...
                                <div className="pt-2">
                                    <span className={getStatusBadgeClasses(invoice.status) + " capitalize"}>
                                        {t(invoice.status)}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                <div className="h-10 w-10 rounded-full bg-primary/10 border border-primary/20 text-primary overflow-hidden flex items-center justify-center text-sm capitalize shadow-sm">
                                    {invoice.vendor?.avatar ? (
                                        <img
                                            src={getImagePath(invoice.vendor.avatar)}
                                    ...
                                    <div className={`${invoice.vendor?.avatar ? 'hidden' : ''} font-bold`}>
                                        {invoice.vendor?.name ? invoice.vendor.name.charAt(0) : '?'}
                                    </div>
                                </div>
                                <div className="min-w-0 flex-1">
                                    <h4 className="font-semibold text-sm text-foreground truncate">{invoice.vendor?.name || t('Guest Vendor')}</h4>
                                    <p className="text-xs text-muted-foreground truncate">{invoice.vendor?.email || t('No email address')}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
