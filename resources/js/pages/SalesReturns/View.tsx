                                    ...
                                    <tbody className="divide-y divide-border">
                                        {salesReturn.items?.map((item, index) => (
                                            <tr key={index} className="hover:bg-muted/10 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-end font-semibold text-foreground">{item.return_quantity || item.quantity}</td>
                        ...
                        <CardContent className="p-6 space-y-4">
                            <div className="flex items-center gap-3">
                                <div className="h-10 w-10 rounded-full bg-primary/10 border border-primary/20 text-primary overflow-hidden flex items-center justify-center text-sm capitalize shadow-sm">
                                    {salesReturn.customer?.avatar ? (
                                        <img
                                            src={getImagePath(salesReturn.customer.avatar)}
                                            alt={salesReturn.customer.name}
                                            className="w-full h-full object-cover"
                                            onError={(e) => {
                                                const target = e.target as HTMLImageElement;
                                                target.style.display = 'none';
                                    ...
                                    <div className={`${salesReturn.customer?.avatar ? 'hidden' : ''} font-bold`}>
                                        {salesReturn.customer?.name ? salesReturn.customer.name.charAt(0) : '?'}
                                    </div>
                                </div>
                                <div className="min-w-0 flex-1">
                                    <h4 className="font-semibold text-sm text-foreground truncate">{salesReturn.customer?.name || t('Guest Customer')}</h4>
                                    <p className="text-xs text-muted-foreground truncate">{salesReturn.customer?.email || t('No email address')}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                            ...
                            {salesReturn.warehouse?.name && (
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <span className="text-sm font-medium text-foreground">{salesReturn.warehouse.name}</span>
                                    </div>
                                </div>
                            )}
