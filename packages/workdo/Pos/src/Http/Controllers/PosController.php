...
class PosController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-pos-orders')) {
            $query = Pos::with(['customer:id,name,email,avatar', 'warehouse:id,name,email', 'payment:pos_id,discount,amount,discount_amount'])
                ->withCount('items')
    ...
    public function show(Pos $sale)
    {
        if (Auth::user()->can('view-pos-orders') && $sale->created_by == creatorId() && ($sale->customer_id == Auth::id() || $sale->creator_id == Auth::id())) {
            $sale->load([
                'customer:id,name,email,avatar,mobile_no',
                'customerDetails',
                'warehouse:id,name',
