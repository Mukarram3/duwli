...
class ShiftController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-shifts')) {
            ...
            // Calculate total counts (independent of pagination/filters)
            $totalShifts = (clone $baseQuery)->count();
            $nightShifts = (clone $baseQuery)->where('is_night_shift', true)->count();
            $dayShifts = (clone $baseQuery)->where('is_night_shift', false)->count();
            ...
            $shifts = $baseQuery
                ...
                ->when(request('shift_type') !== null && request('shift_type') !== '', fn($q) => $q->where('is_night_shift', request('shift_type') === 'night'))
