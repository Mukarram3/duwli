...
class AcknowledgmentController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-acknowledgments')) {
            $baseQuery = Acknowledgment::query()->where(function ($q) {
            ...
            $acknowledgments = (clone $baseQuery)
                ->with(['employee:id,name,email,avatar', 'document', 'assignedBy:id,name,email,avatar'])
                        ...
                        $query->where('acknowledgment_note', 'like', '%' . request('acknowledgment_note') . '%')
                            ->orWhereHas('employee', function ($empQuery) {
                                $empQuery->where('name', 'like', '%' . request('acknowledgment_note') . '%')
                                    ->orWhere('email', 'like', '%' . request('acknowledgment_note') . '%');
                            })
            ...
            return Inertia::render('Hrm/Acknowledgments/Index', [
                ...
                'stats' => $stats,
                'users' => User::whereIn('id', Employee::where('created_by', creatorId())->pluck('user_id'))->select('id', 'name', 'email', 'avatar')->get(),
