...
class EmployeeController extends Controller
{
    ...
    public function index()
    {
        if (Auth::user()->can('manage-employees')) {
            $employees = Employee::query()
                ...
                ->when(request('status') !== null && request('status') !== '', function ($q) {
                    ...
                    $q->whereHas('user', function ($userQuery) use ($status) {
                        if ($status === 'active') {
                            $userQuery->where('is_disable', 0);
                        } elseif ($status === 'inactive') {
                            $userQuery->where('is_disable', 1);
                        }
                    });
                })
            ...
            $fullTime = (clone $statsQuery)->where('employment_type', 'Full Time')->count();
            $partTime = (clone $statsQuery)->where('employment_type', 'Part Time')->count();
            $temporary = (clone $statsQuery)->where('employment_type', 'Temporary')->count();
            $contract = (clone $statsQuery)->where('employment_type', 'Contract')->count();
            
            $activeEmployees = (clone $statsQuery)->whereHas('user', function ($q) {
                $q->where('is_disable', 0);
            })->count();
            $inactiveEmployees = (clone $statsQuery)->whereHas('user', function ($q) {
                $q->where('is_disable', 1);
            })->count();
    ...
    public function edit(Employee $employee)
    {
        if (Auth::user()->can('edit-employees')) {
            ...
            $employee->load(['user:id,name,email,avatar']);
    ...
    public function show(Employee $employee)
    {
        if (Auth::user()->can('view-employees')) {
            ...
            $employee->load(['user:id,name,email,avatar,is_disable', 'branch', 'department', 'designation', 'shift']);
