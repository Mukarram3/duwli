...
export default function Edit() {
    ...
    return (
        <AuthenticatedLayout
            ...
            pageDescription={t('Update employee details, contact information, banking data, and employment documents.')}
            ...
            <Card className="shadow-sm">
                <CardContent className="pt-6">
                    ...
                    <div className="flex items-center gap-4 mb-6 pb-6 border-b dark:border-gray-800">
                        ...
                        />
                        <div>
                            <h2 className="text-lg font-semibold">{employee.user?.name || t('Employee')}</h2>
                        ...
                        <div className="mb-8 border-b pb-6 dark:border-gray-800">
                            <div className="flex flex-wrap items-center justify-between gap-4">
                                {[
                                    ...
                                    { id: 'hours', label: t('Hours & Rates') },
                                    { id: 'documents', label: t('Documents') }
                                ].map((step, index, arr) => {
                                    const isCompleted = arr.findIndex(s => s.id === activeTab) > index;
                                    ...
                                    return (
                                        <div key={step.id} className="flex items-center flex-1 last:flex-initial">
                                            <button
                                                type="button"
                                                onClick={() => {
                                                    if (step.id === 'personal') setActiveTab('personal');
                                                    else if (step.id === 'employment' && validatePersonalTab()) setActiveTab('employment');
                                                    else if (step.id === 'contact' && validatePersonalTab() && validateEmploymentTab()) setActiveTab('contact');
                                                    else if (step.id === 'banking' && validatePersonalTab() && validateEmploymentTab() && validateContactTab()) setActiveTab('banking');
                                            ...
                                            {index < arr.length - 1 && (
                                                <div className="flex-1 mx-4 h-0.5 bg-gray-200 dark:bg-gray-800 hidden xl:block min-w-[30px]" />
                                            )}
                                        </div>
                                    );
                                })}
