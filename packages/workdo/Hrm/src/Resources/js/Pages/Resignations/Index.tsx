...
export default function Index() {
    ...
    const { resignations, auth, employees, stats } = usePage<any>().props;
    ...
    const statsData = stats || { total: 0, pending: 0, accepted: 0, rejected: 0 };
    ...
    const tableColumns = [
                ...
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 border flex items-center justify-center flex-shrink-0">
                        {resignation.employee?.avatar ? (
                            <img src={getImagePath(resignation.employee.avatar)} alt="Avatar" className="w-full h-full object-cover" />
                        ) : (
                            <UserIcon className="w-5 h-5 text-gray-400" />
                        )}
                    </div>
                    <div className="flex flex-col">
                        <span className="font-medium text-sm">{resignation.employee?.name || '-'}</span>
                        <span className="text-xs text-muted-foreground">{resignation.employee?.email || ''}</span>
                    </div>
                </div>
            )
        },
        ...
        },
        {
            ...
            sortable: false,
            render: (_: any, resignation: Resignation) => {
                const user = resignation.approved_by;
                if (!user || !user.name) {
