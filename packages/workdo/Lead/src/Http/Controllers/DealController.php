        ...
        if (Auth::user()->can('view-deals') && $deal->created_by == creatorId()) {
                $deal = Deal::with([
                    ...
                    'userDeals' => function ($query) {
                        $query->with('user:id,name,avatar,email');
                    },
                    'emails',
                    'discussions.creator:id,name,avatar,email',
                    'calls',
                    'files',
                    'activities.user:id,name',
                    'clientDeals.client:id,name,avatar,email'
                ])->find($deal->id);
                ...
                $availableUsers = User::where('created_by', creatorId())
                    ...
                    ->get(['id', 'name']);
                $availableProducts = module_is_active('ProductService') ? ProductServiceItem::where('created_by', creatorId())->get(['id', 'name', 'image', 'sku']) : [];
