...
class CandidateController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-candidates')) {
            ...
            $month = request('month', date('n') - 1); // 0-indexed month (0 = Jan, 11 = Dec)
            ...
            $baseQuery = Candidate::query()
                ...
                ->when(!checkDemo(), function ($q) use ($year, $month) {
                    $q->where(function ($q) use ($year, $month) {
                        $dbMonth = intval($month) + 1;
                        $q->whereYear('application_date', $year)
                          ->whereMonth('application_date', $dbMonth);
                    });
                });

            $totalCandidates = (clone $baseQuery)->count();
            $newCount = (clone $baseQuery)->where('status', '0')->count();
            $screeningCount = (clone $baseQuery)->where('status', '1')->count();
            $interviewCount = (clone $baseQuery)->where('status', '2')->count();
            $offerCount = (clone $baseQuery)->where('status', '3')->count();
            $hiredCount = (clone $baseQuery)->where('status', '4')->count();
            $rejectedCount = (clone $baseQuery)->where('status', '5')->count();
            ...
            $candidates = (clone $baseQuery)
                ...
                ->when(request('application_date') && !checkDemo(), fn($q) => $q->whereDate('application_date', request('application_date')))
    ...
    public function show(Candidate $candidate)
    {
        if (Auth::user()->can('view-candidates')) {
            ...
            $candidate->load(['job_posting.jobType', 'job_posting.location', 'job_posting.branch', 'candidate_source']);
