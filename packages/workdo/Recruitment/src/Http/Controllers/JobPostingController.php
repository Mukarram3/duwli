...
class JobPostingController extends Controller
{
    public function index()
    {
        if(Auth::user()->can('manage-job-postings')){
            ...
            $draftCount = ($statusCounts['draft'] ?? 0) + ($statusCounts['0'] ?? 0);
            $activeCount = ($statusCounts['active'] ?? 0) + ($statusCounts['1'] ?? 0);
            $closedCount = ($statusCounts['closed'] ?? 0) + ($statusCounts['2'] ?? 0);
            ...
            $jobpostings = JobPosting::query()
                ...
                ->when(request('status') && request('status') !== 'all', function($q) {
                    ...
                    if ($status === 'draft') {
                        $q->whereIn('status', ['draft', '0']);
                    } elseif ($status === 'active') {
                        $q->whereIn('status', ['active', '1']);
                    } elseif ($status === 'closed') {
                        $q->whereIn('status', ['closed', '2']);
                    } else {
    ...
    public function store(StoreJobPostingRequest $request)
    {
        if(Auth::user()->can('create-job-postings')){
            ...
            $jobposting->applicant = $validated['applicant'] ?? [];
            $jobposting->visibility = $validated['visibility'] ?? [];
            $jobposting->custom_questions = $validated['custom_questions'] ?? [];
    ...
    public function show(JobPosting $jobposting)
    {
        if(Auth::user()->can('view-job-postings')){
            ...
            // Fetch custom questions associated with this job posting
            $customQuestions = [];
            if ($jobposting && $jobposting->custom_questions) {
                ...
                if (is_array($customQuestionIds) && !empty($customQuestionIds)) {
                    $customQuestions = \Workdo\Recruitment\Models\CustomQuestion::whereIn('id', $customQuestionIds)
