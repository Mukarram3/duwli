...
export default function EditJobPosting({ jobposting, onSuccess }: EditJobPostingProps) {
    ...
    const { data, setData, put, processing, errors, setError, clearErrors } = useForm<EditJobPostingFormData>({
        ...
        visibility: (() => {
            try {
                const vis = jobposting.visibility;
                if (!vis) return [];
        ...
        status: (() => {
            const s = jobposting.status?.toString();
            if (s === 'draft' || s === '0') return '0';
            if (s === 'active' || s === '1') return '1';
            if (s === 'closed' || s === '2') return '2';
            return '0';
        })(),
        ...
        custom_questions: (() => {
            try {
                const cq = jobposting.custom_questions;
                if (!cq) return [];
                if (Array.isArray(cq)) return cq;
                if (typeof cq === 'string') return JSON.parse(cq);
                return [];
            } catch {
                return [];
            }
    ...
    const validateStep1 = () => {
        let isValid = true;
        const requiredFields: (keyof EditJobPostingFormData)[] = [
            'title', 'job_type_id', 'location_id', 'branch_id', 'job_application', 'position', 'priority', 'application_deadline', 'status'
        ];
    ...
    return (
        <AuthenticatedLayout
            ...
            pageTitle={t('Edit Job Posting')}
            pageDescription={t('Update the details below to modify this job posting.')}
            backUrl={route('recruitment.job-postings.index')}
        >
            <Head title={t('Edit Job Posting')} />

            <div className="w-full">
                {/* Outer Card */}
                <Card className="border border-gray-300 dark:border-zinc-700 shadow-sm overflow-visible bg-white dark:bg-zinc-955 p-6 md:p-8 space-y-6">
                    ...
                    <div className="relative w-full px-4 mb-4">
                        ...
                        <div className="flex items-center justify-between relative z-10">
                            {steps.map((step) => {
                                ...
                                return (
                                    <div key={step.number} className="flex flex-col items-center cursor-pointer group" onClick={() => setCurrentStep(step.number)}>
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${hasError
                                            ...
                                            : isCompleted
                                                ? 'bg-primary border-primary text-white group-hover:bg-primary/90 group-hover:border-primary/90'
                                                : isActive
                                                    ? 'bg-white dark:bg-zinc-955 border-primary text-primary font-semibold shadow-sm'
                                                    : 'bg-gray-100 dark:bg-zinc-900 border-gray-300 dark:border-zinc-800 text-gray-400 dark:text-zinc-600 group-hover:border-gray-400 dark:group-hover:border-zinc-500'
                                            }`}>
                                        ...
                                        <span className={`text-xs mt-2 font-semibold text-center max-w-[80px] md:max-w-none transition-colors duration-300 ${hasError
                                            ...
                                            : isActive
                                                ...
                                                : isCompleted
                                                    ? 'text-primary/80 group-hover:text-primary/90'
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            <div>
                                                ...
                                                <Select value={data.status?.toString() || ''} onValueChange={(value) => setData('status', value)} required>
                                                    <SelectTrigger>
                                                    ...
                                                    <SelectContent>
                                                        ...
                                                        <SelectItem value="2">{t('Closed')}</SelectItem>
                                                    </SelectContent>
                                                </Select>
