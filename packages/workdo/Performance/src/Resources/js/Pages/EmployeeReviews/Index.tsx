// packages/workdo/Performance/src/Resources/js/Pages/EmployeeReviews/Index.tsx
            ...
            render: (_: any, item: EmployeeReview) => (
                <div className="flex items-center gap-2">
                    <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 border flex items-center justify-center flex-shrink-0">
                        {item.user?.avatar ? (
                            <img src={getImagePath(item.user.avatar)} alt="Avatar" className="w-full h-full object-cover" />
                        ) : (
                ...
                <div className="flex items-center gap-2">
                    <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 border flex items-center justify-center flex-shrink-0">
                        {item.reviewer?.avatar ? (
                            <img src={getImagePath(item.reviewer.avatar)} alt="Avatar" className="w-full h-full object-cover" />
                        ) : (
                            <UserIcon className="w-5 h-5 text-gray-400" />
                        )}
                    </div>
                    <div className="min-w-0">
                        <p className="font-medium text-sm">{item.reviewer?.name || '-'}</p>
                        <p className="text-xs text-muted-foreground truncate">{item.reviewer?.email || ''}</p>
                    </div>
                </div>
            )
        },
            ...
            <AuthenticatedLayout
                ...
                pageActions={
                    <PageActionBar
                        actions={[
                            {
                                label: t('Create Employee Review'),
                                onClick: () => openModal('add'),
                                icon: Plus,
                                variant: 'primary',
                                permission: 'create-employee-reviews',
                            },
                            // Goals, cycles and indicators, moved off the sidebar.
                            ...getRelatedActions('performance.employee-reviews.index', t),
                        ]}
                                                ...
                                                <div className="p-4 bg-gradient-to-r from-primary/5 to-transparent border-b flex-shrink-0">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-12 h-12 rounded-lg overflow-hidden bg-gray-100 border flex-shrink-0 flex items-center justify-center">
                                                            {review.user?.avatar ? (
                                                                <img src={getImagePath(review.user.avatar)} alt="Avatar" className="w-full h-full object-cover" />
                                                            ) : (
                                                                <UserIcon className="w-6 h-6 text-primary" />
                                                            )}
                                                        </div>
                                                        <div className="min-w-0 flex-1">
                                                            <h3 className="font-semibold text-sm text-gray-900 truncate">{review.user?.name || 'N/A'}</h3>
