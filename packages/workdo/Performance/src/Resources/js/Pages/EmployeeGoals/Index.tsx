...
import { Plus, Edit, Trash2, Eye, Target, User as UserIcon } from "lucide-react";
                ...
                <div className="flex items-center gap-2">
                    <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 border flex items-center justify-center flex-shrink-0">
                        {goal.employee?.avatar ? (
                            <img src={getImagePath(goal.employee.avatar)} alt="Avatar" className="w-full h-full object-cover" />
                        ) : (
                            <UserIcon className="w-5 h-5 text-gray-400" />
                        )}
                    </div>
                    <div className="min-w-0">
                        <p className="font-medium text-sm">{goal.employee?.name || '-'}</p>
                        <p className="text-xs text-muted-foreground truncate">{goal.employee?.email || ''}</p>
                    </div>
                </div>
            )
        },
                                        ...
                                        {goals.data.map((goal) => (
                                            <Card key={goal.id} className="p-0 hover:shadow-lg transition-all duration-200 flex flex-col h-full min-w-0">
                                                ...
                                                <div className="p-4 bg-gradient-to-r from-primary/5 to-transparent border-b flex-shrink-0">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-12 h-12 rounded-lg overflow-hidden bg-gray-100 border flex-shrink-0 flex items-center justify-center">
                                                            {goal.employee?.avatar ? (
                                                                <img src={getImagePath(goal.employee.avatar)} alt="Avatar" className="w-full h-full object-cover" />
                                                            ) : (
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            <h3 className="font-semibold text-sm text-gray-900 truncate">{goal.employee?.name || '-'}</h3>
                                                            <p className="text-xs text-muted-foreground truncate">{goal.employee?.email || ''}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="p-4 flex-1 min-h-0">
                                                    <div className="grid grid-cols-2 gap-4 mb-4">
                                                        <div className="text-xs min-w-0">
                                                            <p className="text-muted-foreground mb-1 text-xs capitalize tracking-wide">{t('Employee')}</p>
                                                            <p className="font-medium text-xs">{goal.employee?.name || '-'}</p>
                                                        </div>
                                                        <div className="text-xs min-w-0">
                                                            <p className="text-muted-foreground mb-1 text-xs capitalize tracking-wide">{t('Type')}</p>
                                                    ...
                                                    <div className="grid grid-cols-2 gap-4 mb-4">
                                                        <div className="text-xs min-w-0">
                                                            <p className="text-muted-foreground mb-1 text-xs capitalize tracking-wide">{t('Target')}</p>
                                                            <p className="font-medium text-xs">{goal.target}</p>
                                                        </div>
                                                        <div className="text-xs min-w-0">
                                                            <p className="text-muted-foreground mb-1 text-xs capitalize tracking-wide">{t('Due Date')}</p>
                                                    ...
                                                    <div className="mb-4">
                                                        <div className="flex items-center justify-between mb-1">
                                                            <p className="text-muted-foreground text-xs capitalize tracking-wide">{t('Progress')}</p>
