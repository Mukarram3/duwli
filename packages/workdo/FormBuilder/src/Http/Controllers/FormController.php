...
class FormController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-formbuilder')) {
            ...
            $totalForms = (clone $baseQuery)->count();
            $totalFields = (clone $baseQuery)->withCount('fields')->get()->sum('fields_count');
            $totalResponses = (clone $baseQuery)->withCount('responses')->get()->sum('responses_count');
            $avgResponse = $totalForms > 0 ? round($totalResponses / $totalForms, 2) : 0;
