    ...
    public function index()
    {
        if (Auth::user()->can('manage-contracts')) {
            ...
            $cardStats = [
                ...
                'active' => (clone $baseQuery)->whereIn('status', ['accepted', 'active'])->count(),
                'pending' => (clone $baseQuery)->whereIn('status', ['pending', 'draft'])->count(),
                'closed' => (clone $baseQuery)->where('status', 'closed')->count(),
                'declined' => (clone $baseQuery)->where('status', 'declined')->count(),
            ];

            $contract = $baseQuery->with(['user', 'contractType'])
