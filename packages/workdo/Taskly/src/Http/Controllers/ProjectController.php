...
class ProjectController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-project')) {
            ...
            $items->getCollection()->transform(function ($project) {
                ...
                $project->completed_task_count = ProjectTask::where('project_id', $project->id)->whereHas('taskStage', function($q) {
            ...
            // Stats — based on all accessible projects (unfiltered)
            $statsQuery = Project::query()
                ->where(function($q) {
                    if(Auth::user()->can('manage-any-project')) {
                        ...
                        $q->where(function($subQ) {
                            $subQ->where('creator_id', Auth::id());
                            if (Auth::user()->type === 'client') {
                                $subQ->orWhereHas('clients', fn($c) => $c->where('client_id', Auth::id()));
                            } else {
                                $subQ->orWhereHas('teamMembers', fn($t) => $t->where('user_id', Auth::id()));
                            }
                        });
                    } else {
                        $q->whereRaw('1 = 0');
                ...
                ->get(['id', 'status', 'end_date']);

            $stats = [
                'total'    => $statsQuery->count(),
                'ongoing'  => $statsQuery->where('status', 'Ongoing')->count(),
                'onhold'   => $statsQuery->where('status', 'Onhold')->count(),
                'finished' => $statsQuery->where('status', 'Finished')->count(),
                'overdue'  => $statsQuery->filter(fn($p) => $p->end_date && \Carbon\Carbon::parse($p->end_date)->isPast())->count(),
            ];
    ...
    public function show(Project $project)
    {
        if (Auth::user()->can('view-project') && $project->created_by == creatorId()) {
            $project->load(['teamMembers:id,name,avatar,email', 'clients:id,name,avatar,email', 'milestones']);
