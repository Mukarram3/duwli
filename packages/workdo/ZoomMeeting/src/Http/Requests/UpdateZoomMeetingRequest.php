...
class UpdateZoomMeetingRequest extends FormRequest
{
    ...
    {
        $zoommeeting = $this->route('zoommeeting');
        $startTimeRule = 'required|date';

        if ($zoommeeting) {
            $requestTime = $this->input('start_time') ? strtotime($this->input('start_time')) : null;
            $dbTime = $zoommeeting->start_time ? strtotime($zoommeeting->start_time) : null;

            if ($requestTime !== $dbTime) {
                $startTimeRule .= '|after:' . date('Y-m-d H:i:s', time() - 60);
            }
        } else {
            $startTimeRule .= '|after:' . date('Y-m-d H:i:s', time() - 60);
        }
