...
class ZoomMeetingController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-zoom-meetings')) {
            ...
            $zoommeetings = $zoommeetings->when(request('sort'), fn($q) => $q->orderBy(request('sort'), request('direction', 'asc')), fn($q) => $q->latest())
                ->paginate(request('per_page', 100))
                ->withQueryString();
            if (checkDemo()) {
                foreach ($zoommeetings as $key => $meeting) {
                    $day = ($key % 28) + 1;
                    $hour = 9 + (($key % 5) * 2); // 9, 11, 13, 15, 17
                    $minute = ($key % 2) * 30; // 0, 30

                    $newTime = now()->setDate((int)$year, (int)$month, $day)->setTime($hour, $minute, 0);
                    $meeting->start_time = $newTime->format('Y-m-d H:i:s');
                }
            }
