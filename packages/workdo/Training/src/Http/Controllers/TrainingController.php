...
class TrainingController extends Controller
{
    public function index()
    {
        if(Auth::user()->can('manage-trainings')){
            ...
            // Calculate stats based on current branch, department, and training_type filters
            $statsQuery = Training::query()
                ->where(function($q) {
                    if(Auth::user()->can('manage-any-trainings')) {
                        $q->where('created_by', creatorId());
                    } elseif(Auth::user()->can('manage-own-trainings')) {
                ...
                ->when(request('branch_id'), fn($q) => $q->where('branch_id', request('branch_id')))
                ->when(request('department_id'), fn($q) => $q->where('department_id', request('department_id')))
                ->when(request('training_type_id'), function($q) {
                    $typeId = request('training_type_id');
                    if ($typeId && $typeId !== '_none_') {
                        $q->where('training_type_id', $typeId);
                    }
                });
            ...
            $stats = [
                ...
                'scheduled' => (int) ($allStats->scheduled ?? 0),
                'ongoing' => (int) ($allStats->ongoing ?? 0),
    ...
    public function store(StoreTrainingRequest $request)
    {
        if(Auth::user()->can('create-trainings')){
            ...
            return back()->with('success', __('The training list has been created successfully.'));
        }
        else{
            return back()->with('error', __('Permission denied'));
        }
