    ...
    public function index()
    {
        if(Auth::user()->can('manage-goal-tracking')){
            ...
            $stats = [
                ...
                'ahead' => (clone $baseQuery)->where('on_track_status', 'ahead')->count(),
                'on_track' => (clone $baseQuery)->where('on_track_status', 'on_track')->count(),
                'behind' => (clone $baseQuery)->where('on_track_status', 'behind')->count(),
                'critical' => (clone $baseQuery)->where('on_track_status', 'critical')->count(),
                'total_contribution' => (clone $baseQuery)->sum('contribution_amount')
            ];
