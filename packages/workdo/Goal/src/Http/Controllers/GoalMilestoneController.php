    ...
    public function index()
    {
        if(Auth::user()->can('manage-goal-milestones')){
            ...
            $stats = [
                ...
                'achieved' => (clone $baseQuery)->where('status', 'achieved')->count(),
                'pending' => (clone $baseQuery)->where('status', 'pending')->count(),
                'overdue' => (clone $baseQuery)->where('status', '!=', 'achieved')->whereDate('target_date', '<', $todayStr)->count(),
                'total_target' => (clone $baseQuery)->sum('target_amount'),
                'total_achieved' => (clone $baseQuery)->sum('achieved_amount')
            ];
